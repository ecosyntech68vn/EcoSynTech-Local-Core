...
router.get('/sensor-history', authenticate, asyncHandler(async (req: any, res: any) => {
...
router.get('/dashboard', authenticate, asyncHandler(async (req: any, res: any) => {
  ...
  res.json({
    ...
    dashboard: {
      ...
      sensors: sensors.reduce((acc: any, s: any) => {
...
router.get('/kpis', authenticate, asyncHandler(async (req: any, res: any) => {
...
router.get('/export/excel', authenticate, asyncHandler(async (req: any, res: any) => {
  ...
  getAll('SELECT * FROM sensors').forEach((s: any) => sensorsSheet.addRow(s));
  ...
  getAll('SELECT * FROM devices').forEach((d: any) => devicesSheet.addRow(d));
  ...
  getAll('SELECT * FROM alerts ORDER BY timestamp DESC LIMIT 100').forEach((a: any) => alertsSheet.addRow(a));
