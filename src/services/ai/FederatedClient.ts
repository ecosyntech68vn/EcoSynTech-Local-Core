...
import DEFAULT_SERVER_URL = process.env.FEDERATED_SERVER_URL || 'http://localhost:5050';
import CLIENT_ID = process.env.FARM_CLIENT_ID || `farm_${Date.now()}`;

import federatedBreaker = getBreaker('federated-client', {
  failureThreshold: 5,
  timeout: 60000,
  resetTimeout: 120000
});
