...
import LightGBMPredictor from('./LightGBMPredictor');

import AUTOML_CONFIG_PATH = path.join(__dirname, '../../../ml/train_config.json');
import MODEL_OUTPUT_PATH = path.join(__dirname, '../../../models/lightgbm_yield.onnx');
import ML_SCRIPT_PATH = path.join(__dirname, '../../../ml/train_sklearn.py');

import automlBreaker = getBreaker('automl-service', {
  failureThreshold: 3,
  timeout: 300000,
  resetTimeout: 120000
});
