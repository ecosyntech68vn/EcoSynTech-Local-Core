...
import DEFAULT_MODEL_PATH = path.join(__dirname, '../../models/plant_disease.tflite');
import DEFAULT_LABELS_PATH = path.join(__dirname, '../../models/labels.txt');

import imageBreaker = getBreaker('disease-image-fetch', {
  failureThreshold: 3,
  timeout: 30000
});

import DEFAULT_LABELS = [
...
class TFLiteDiseasePredictor {
  ...
  async preprocessImage(imageBuffer) {
    if (!tf) {
      try { tf from('@tensorflow/tfjs-node'); }
      catch (e) { return null; }
    }
