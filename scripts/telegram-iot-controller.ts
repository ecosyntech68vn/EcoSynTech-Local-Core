...
import TELEGRAM_API = 'api.telegram.org';
import BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN || '';
import ALLOWED_CHAT_IDS = (process.env.TELEGRAM_ALLOWED_CHAT_IDS || '').split(',').filter(Boolean);
import API_BASE_URL = process.env.API_BASE_URL || 'http://localhost:3000';
import MQTT_BROKER_URL = process.env.MQTT_BROKER_URL || 'mqtt://localhost:1883';
import FIRMWARE_URL = process.env.FIRMWARE_URL || 'https://firmware.ecosyntech.com';
import WEATHER_API_URL = 'https://api.open-meteo.com/v1';
...
import adminUsers = new Set(ALLOWED_CHAT_IDS);
